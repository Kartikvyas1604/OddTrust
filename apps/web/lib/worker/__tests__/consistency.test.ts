...
import { checkConsistency, computeArbitrageStakes } from '../consistency';
import { computeImpliedProbability, oddsToProbability, normalizeMargin, getBookmakerMargin } from '../market';
...
describe('checkConsistency', () => {
  ...
  it('detects inconsistent market (arbitrage) — Σ(1/odds) < 1', () => {
    const markets: MarketOdds[] = [
      { type: 'match_winner', outcomes: { home: 3.0, draw: 4.0, away: 3.2 } },
    ];
...
describe('computeArbitrageStakes', () => {
  it('computes stakes for arbitrage with 100 unit total', () => {
    const markets: MarketOdds[] = [
      { type: 'match_winner', outcomes: { home: 2.5, draw: 3.5, away: 2.8 } },
    ];
  ...
  it('returns empty object for consistent markets', () => {
    const markets: MarketOdds[] = [
      { type: 'match_winner', outcomes: { home: 2.0, draw: 3.5, away: 3.5 } },
    ];
    const stakes = computeArbitrageStakes(markets);
...
describe('checkConsistency — known fixtures', () => {
  it('computes Σ(1/odds) for a 3-outcome market', () => {
    const markets: MarketOdds[] = [
      { type: 'match_winner', outcomes: { home: 2.10, draw: 3.25, away: 3.50 } },
    ];
  ...
  it('detects arbitrage with over/under market', () => {
    const markets: MarketOdds[] = [
      { type: 'over_under_2.5', outcomes: { over: 2.25, under: 1.65 } },
    ];
  ...
  it('computes margin correctly', () => {
    const markets: MarketOdds[] = [
      { type: 'both_teams_score', outcomes: { yes: 1.8, no: 2.0 } },
    ];
